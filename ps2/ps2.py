...
# Answer:
#
# A node is a building.
# An edge is the connection between each building/node.
# The weighted edge values are the distance between the 2 buildings(nodes)
# + the distance outdoors between both them.
...
def load_map(map_filename):
    ...
    #         src, dest, int(edges[2]), int(edges[3].rstrip())))
    # return digraph
    ...
    with open(map_filename) as file:
        for line in file:
            ...
            edge = WeightedEdge(src, dest, *map(int, (total, outdoor)))
...
# Constraint is the maximum distance limit outdoors.
    ...
    for edge in digraph.edges[digraph.get_node(start)]:
        ...
        if node not in path[0]:
            ...
            new_path = get_best_path(
                digraph, node, end, updated_path, max_dist_outdoors, best_dist, best_path)
            if new_path:
...
def directed_dfs(digraph, start, end, max_total_dist, max_dist_outdoors):
    ...
    """
    path = [[], 0, 0]
    best_path = get_best_path(digraph, start, end, path,
                              max_dist_outdoors, best_dist=None, best_path=None)
    if best_path[1] is None:
        print('There is no Best Path that meets constraints!')
        raise ValueError
    else:
        if best_path[1] > max_total_dist:
            print('Best Path exceeded max_total_dist')
            raise ValueError
        else:
            return best_path[0]
